...
logMessage("HTML 5 Video Playback Control (v1.6)");


const fetchVideoTimeout = setInterval(fetchVideoCallback, 500);
let videoElements = [];

function fetchVideoCallback() {
    logMessage("Attempting to fetch video...");
    videoElements = document.getElementsByTagName("video");

    if (videoElements.length > 0) {
        logMessage("Successfully fetched video tags.");
        clearInterval(fetchVideoTimeout);
        entryPoint();
    }    
}
